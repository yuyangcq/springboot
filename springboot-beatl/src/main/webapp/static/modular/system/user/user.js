...
MgrUser.initColumn = function () {
    return [
        ...
        {title: '应用名称', field: 'appname', align: 'center', valign: 'middle', sortable: true},
...
MgrUser.roleAssign = function () {
    ...
    if (this.check()) {
        this.layerIndex = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            /** @namespace this.seItem.appid */
            content: Feng.ctxPath + '/mgr/role_assign/' + this.seItem.id + '/' + this.seItem.appid
        });
    }
};
...
MgrUser.freezeAccount = function () {
    if (this.check()) {
        ...
        var ajax = new $ax(Feng.ctxPath + "/mgr/freeze", function () {
            Feng.success("冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
...
MgrUser.unfreeze = function () {
    if (this.check()) {
        var userId = this.seItem.id;
        var ajax = new $ax(Feng.ctxPath + "/mgr/unfreeze", function () {
            Feng.success("解除冻结成功!");
            MgrUser.table.refresh();
        }, function () {
            Feng.error("解除冻结失败!");
...
MgrUser.resetPwd = function () {
    if (this.check()) {
        ...
        parent.layer.confirm('是否重置密码为111111？', {
            ...
            var ajax = new $ax(Feng.ctxPath + "/mgr/reset", function () {
                Feng.success("重置密码成功!");
            }, function () {
                Feng.error("重置密码失败!");
            });
...
MgrUser.search = function () {
    ...
    var ztree = new $ZTree("deptTree", "/dept/tree/" + queryData['appid']);
...
$(function () {
    ...
    queryData['appid'] = appid.val();
    MgrUser.table.refresh({query: queryData});
    var ztree = new $ZTree("deptTree", "/dept/tree/1");
    ztree.bindOnClick(MgrUser.onClickDept);
    ztree.init();
    appid.find("option[value='1']").attr("selected", true);
});
