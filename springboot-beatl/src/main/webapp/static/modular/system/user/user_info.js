        ...
        password: {
            validators: {
                ...
                regexp: {
                    ...
                    message: '密码不少于8位，至少由大写字母、小写字母和阿拉伯数字组成'
                },
            }
        },
        rePassword: {
...
UserInfoDlg.showDeptSelectTree = function () {
	...
	debugger;
	var appid=$("#appid").val();
	if(appid==undefined||appid==''||appid=='0'){
		return Feng.error("请选择应用名称!");
	}
	
    var ztree = new $ZTree("treeDemo", "/dept/tree/"+appid);
...
UserInfoDlg.collectData = function () {
    this.set('id').set('appid').set('account').set('sex').set('password').set('avatar')
 ...
 * 验证数据是否为空
 */
UserInfoDlg.validatePwdForm = function () {
...
$(function () {
    ...
    Feng.initValidator("userPwdForm", UserInfoDlg.validateFields);
