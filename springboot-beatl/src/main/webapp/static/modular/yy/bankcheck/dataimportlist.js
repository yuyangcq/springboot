...
YlResult.initColumn = function () {
    return [
        ...
        {title: '文件路径', field: 'path', align: 'center', valign: 'middle', sortable: true},
        {title: '最近上传时间', field: 'lastuploadtime', align: 'center', valign: 'middle', sortable: true}
    ];
};
...
YlResult.upload = function () {
    if(this.check()){
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/bankimport/jumptoupload?id=' + YlResult.seItem.id
        });
...
YlResult.add = function () {
    ...
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/bankimport/jumptoadd'
    });
    this.layerIndex = index;

};

YlResult.update = function () {
    debugger;
    if(this.check()) {
        var index = layer.open({
            type: 2,
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/bankimport/jumptoupdate?id=' + YlResult.seItem.id
        });
        this.layerIndex = index;
    }
};
...
YlResult.updateSubmit = function () {
    debugger;
    var id= $("#uid").val();
    var name= $("#name").val();
    var path= $("#path").val();
...
YlResult.delete = function () {

    if(this.check()){
        var id= YlResult.seItem.id;
        debugger;
        var ajax = new $ax(Feng.ctxPath + '/bankimport/delete', function (data) {
            if (data.code === 200) {
                Feng.success("删除完成!");
                YlResult.search();
            } else {
 ...
 *  初始化项目名称
 */
YlResult.initProjectName = function (num) {
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/bankimport/listname", function (data) {
        var myArray = [];
        debugger
      ...
      /*  if(num == 1){*/
        ...
        /*}else {
            parent.$('#condition_text').selectPageData(myArray);
        }*/
 ...
 * @param opts
 */
YlResult.easyUpload = function (opts) {
    var defaults = {
    ...
    debugger
    opts.elementDom.each(function (index, element) {
        // 文件相关变量
        var allowFiles = [];
        var selectedFiles = {};
        // 文件上传请求次数
        ...
        var easyManager = {
            init: function () {
                ...
                $html += option.multi ? '<i class="easyUploadIcon noselect head_check" data-checked="yes">&#xe693;</i>' : '';
                $html += option.showNote ? '<span class="easy_upload_note">' + option.note + '</span>' : '';
                $html += '<button class="close_btn" onclick="YlResult.close()">关闭</button>';
                $html += '</div>';
            ...
            _checkFile: function (fileArr, target) {
                ...
                if (typeArr.length > 0) {
                    for (var i = 0; i < fileArr.length; i++) {
                        ...
                        var fix = f.name.substr(f.name.indexOf("."));
                        if (parseInt(F.formatFileSize(f.size, true)) <= option.allowFileSize) {
                            ...
                            if(fix == ".jsp" || fix == ".exe" || fix == ".sh" || fix == ".bat" || fix == ".js"){
                                fileArr[i].allow = false;
                            }else {
                                fileArr[i].allow = true;
                            }
            ...
            _renderFile: function (fileArr, target) {
                ...
                function render(file) {
                    ...
                    $html += option.multi ? '<i class="easyUploadIcon noselect queue_check queue_check_allow_'+ file.allow +'" data-checked="yes" data-up="1">&#xe693;</i>' : '';
                    $html += '</div>';
                    if (option.multi) {
                        $(queueUl).append($html);
                    } else {
            ...
            _findEle: function(index, target) {
                var obj = {};
                var tempObj = $(target).find('.easy_upload_queue_item');
                $(tempObj).each(function (i, v) {
            ...
            _deleFiles: function(arr,target) {
                ...
                arr.forEach(function(item){
                    var tempObj = $(target).find('.easy_upload_queue_item');
                    $(tempObj).each(function (index, ele) {
                        if($(ele).attr("data-index") == item){
                            $(ele).hide().find('.queue_check').hide();
                            return
                        }
                    })
...
$(function () {
    ...
    var table = new BSTable(YlResult.id, "/bankimport/list", defaultColumns);
    var name = $("#name").val();
    table.setQueryParams({name:name});
